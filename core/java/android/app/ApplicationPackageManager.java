...
public class ApplicationPackageManager extends PackageManager {
    ...
    public boolean hasSystemFeature(String name, int version) {
        if (name != null && Arrays.asList(featuresTensor).contains(name)
                && !Arrays.asList(pTensorCodenames).contains(SystemProperties.get("ro.cherish.device"))) {
            return false;
        }
        String packageName = ActivityThread.currentPackageName();
        if ((!Arrays.asList(pTensorCodenames).contains(SystemProperties.get("ro.cherish.device"))
